# myredbit-app
Single page To-do app on Vanilla Js 

----Disclaimer----
myredbit-app is my very first web app project after 4 months learning Javascript, HTML, and CSS.
So i will be so happy if you have time to read my code, or to try the app, or maybe share your thoughts whether it's a critics, advices, or questions about this project.

----Features----

1. Single Page App. 
2. Book Registration.
3. Book Editing.
4. Book Status Modification.
5. Book Notes.
6. Search Books.
7. Filter and sort Books.

----Notes----
There is so many try catch operation because i recently just learn about JavaScript Hoist, so sometimes command a function to get an element before it's loaded so the app 
not working properly, with try catch operation i can easily debug which elements that is error.

